...
@pytest.fixture
def sample_event():
    message_data = {"action": "load_standings_to_bigquery"}
...
def test_load_standings_to_bigquery_success(sample_event, sample_context):
    with (
        patch.dict(
            "os.environ",
        ...
        result_message, status_code = load_standings_to_bigquery(
            sample_event, sample_context
        )

        assert status_code == 200
        assert "External table 'standings_parquet' has been updated." in result_message
        assert "Total standings records available: 100" in result_message
...
def test_load_standings_to_bigquery_existing_table(sample_event, sample_context):
    with (
        patch.dict(
            "os.environ",
            {"BUCKET_NAME": "test-bucket", "GCP_PROJECT_ID": "test-project"},
        ...
        assert status_code == 200
        assert "External table 'standings_parquet' has been updated." in result_message
        assert "Total standings records available: 200" in result_message
...
def test_load_standings_to_bigquery_invalid_message():
    event = {
    ...
    result_message, status_code = load_standings_to_bigquery(event, context)
...
def test_load_standings_to_bigquery_bigquery_exception(sample_event, sample_context):
    with (
        ...
        assert status_code == 500
        assert "Error during Standings BigQuery external table update" in result_message
