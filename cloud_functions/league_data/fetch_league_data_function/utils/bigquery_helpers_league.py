...
if not isinstance(project_id, str):
    raise ValueError(
        "Project ID must be a string. Please set GOOGLE_CLOUD_PROJECT environment variable."
    )
...
def insert_data_into_bigquery(table_name: str, data: List[Dict[str, Any]]) -> None:
    ...
    delete_query = f"DELETE FROM `{table_id}` WHERE CAST(id AS STRING) IN ({id_list})"  # nosec B608
    ...
    if load_job.errors:
        logging.error(
            f"Errors occurred while inserting into {table_name}: {load_job.errors}"
        )
    else:
