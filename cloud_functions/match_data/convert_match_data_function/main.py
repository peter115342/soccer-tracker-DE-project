    ...
    try:
        ...
        if "action" not in message_data or message_data["action"] != "convert_matches":
            ...
            send_discord_notification(
                "❌ Convert to Parquet: Invalid Trigger", error_message, 16711680
            )
        ...
        if not json_files:
            ...
            send_discord_notification(
                "📝 Convert to Parquet: No Files", message, 16776960
            )

            publisher = pubsub_v1.PublisherClient()
            weather_topic_path = publisher.topic_path(
                os.environ["GCP_PROJECT_ID"], "fetch_weather_data_topic"
            )

            weather_message = {"action": "fetch_weather"}

            future = publisher.publish(
                weather_topic_path, data=json.dumps(weather_message).encode("utf-8")
            )

            publish_result = future.result()
            logging.info(
                f"Published no-files message to fetch_weather_data_topic with ID: {publish_result}"
            )
        ...
        send_discord_notification(
            "✅ Convert to Parquet: Complete", status_message, 65280
        )

        publisher = pubsub_v1.PublisherClient()
        bigquery_topic_path = publisher.topic_path(
            os.environ["GCP_PROJECT_ID"], "match_to_bigquery_topic"
        )

        bigquery_message = {"action": "load_matches_to_bigquery"}
        ...
        publish_result = future.result()
        logging.info(
            "Published message to match_to_bigquery_topic with conversion stats"
        )
        ...
        send_discord_notification(
            "❌ Convert to Parquet: Failure", error_message, 16711680
        )
