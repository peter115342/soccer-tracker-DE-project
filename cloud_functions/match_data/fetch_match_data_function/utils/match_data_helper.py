...
def fetch_matches_for_competitions(
    ...
    for competition in COMPETITION_CODES:
        ...
        try:
            ...
            logging.info(
                f"Competition {competition}: Found {len(matches)} matches, {new_matches_count} are new"
            )
    ...
    logging.info(
        f"Total matches found: {total_matches_found}, New matches to process: {new_matches_count}"
    )
    return all_matches
