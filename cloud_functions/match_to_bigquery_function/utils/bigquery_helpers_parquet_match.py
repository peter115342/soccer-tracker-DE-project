...
def preprocess_parquet_files(
    ...
    for file in files:
        ...
        if next(client.query(query, job_config=job_config_query).result()).count > 0:
            logging.info(f"Match {match_id} already exists in BigQuery, skipping preprocessing...")
            continue
        ...
        with tempfile.NamedTemporaryFile(suffix='.parquet') as temp_file:
            ...
            df = pl.read_parquet(temp_file.name)
                ...
                try:
                    def replace_null_nationality(refs):
                        if isinstance(refs, list):
                            return [
                                {**ref, 'nationality': ref.get('nationality') or 'None'}
                                for ref in refs
                            ]
                    ...
                    logging.warning(f"Error processing referees for match {match_id}: {str(e)}")
                    pass
        ...
        logging.info(f"Preprocessed match {match_id} parquet file.")
